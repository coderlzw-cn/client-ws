interface Options {
    ...
    reconnectInterval?: number;         // 重连间隔，5s
...
export default class ClientWs {
    ...
    readonly #sendMessageQueue: Record<string, sendMessageQueueItem> = {};  // ws 断开后，缓存的消息
    #isHandleClose: boolean = false;  // 是否手动关闭的连接，手动关闭的连接不执行重连操作
        ...
        if (this.#options.timeout) {
            setTimeout(() => {
                ...
                if (this.#ws && this.#ws.readyState !== WebSocket.OPEN) {
                    this.#ws.close();
                    this.emit("error", new Event("connect timeout"));
    ...
    private reconnect() {
        if (!this.#ws) return;
        // 如果设置了最大重连次数，并且已经达到最大重连次数，则直接抛出异常
